...
class Symfony_Sniffs_NamingConventions_ValidFunctionNameSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff
{
    ...
    /**
     * Constructs a Symfony_Sniffs_NamingConventions_ValidFunctionNameSniff.
     */
    ...
    protected function processTokenWithinScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $currScope)
    {
        ...
        // If it's a private method, it should be in camel caps format
        if ($isPublic === false) {
            if (PHP_CodeSniffer::isCamelCaps($methodName) === false)
            {
                $error = "Private method name \"$className::$methodName\" is not in camel caps format";
                $phpcsFile->addError($error, $stackPtr);
            }
            return;
        }
